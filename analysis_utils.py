...
def detect_x_column(df: pd.DataFrame, min_valid_points: int = 3) -> Tuple[str, List[str]]:
    ...
    for column in columns:
        numeric_counts[column] = int(pd.to_numeric(df[column], errors="coerce").notna().sum())

    numeric_columns = [column for column in columns if numeric_counts[column] >= min_valid_points]
    if not numeric_columns:
        raise ValueError("No numeric-like columns were found for wavelength (x-axis).")
    ...
    if keyword_candidates:
        x_column = max(keyword_candidates, key=lambda c: numeric_counts[c])
    ...
    if first_column in numeric_columns:
        notes.append(
            "No wavelength-like header detected; using the first column as x-axis by default."
        )
        return first_column, notes

    x_column = max(numeric_columns, key=lambda c: numeric_counts[c])
    notes.append(
        f"No wavelength-like header detected; using numeric column '{x_column}' with the highest valid count."
    )
    return x_column, notes
...
def extract_series_data(
    ...
    finite_mask = x_numeric.notna() & y_numeric.notna()
    x_values = x_numeric[finite_mask].to_numpy(dtype=float)
    y_values = y_numeric[finite_mask].to_numpy(dtype=float)

    notes: List[str] = []
    dropped_rows = int(len(df) - finite_mask.sum())
    if dropped_rows > 0:
        notes.append(f"Dropped {dropped_rows} row(s) with non-numeric x/y in '{y_column}'.")
