...
def default_series_color_for_name(series_name: str) -> str:
    digest = hashlib.md5(series_name.encode("utf-8")).hexdigest()
    index = int(digest[:8], 16) % len(SERIES_STYLE_PALETTE)
    return SERIES_STYLE_PALETTE[index]
...
def resolve_plot_title(base_title: str, settings: Optional[PlotCustomizationSettings]) -> str:
    ...
    heading = settings.figure_heading.strip()
    if not heading:
        return base_title
    if not base_title:
        return heading
    return f"{heading} | {base_title}"
...
def resolve_series_line_color(
    ...
    candidate = settings.series_line_colors.get(series_name, fallback_color)
    if candidate is None:
        return None
    if is_valid_matplotlib_color(candidate):
        return candidate
    return fallback_color
...
def _resolve_font_sizes(settings: PlotCustomizationSettings) -> Tuple[Optional[float], Optional[float], Optional[float], Optional[float]]:
    title_size = settings.title_font_size if settings.title_font_size is not None else settings.font_size_global
    axis_label_size = settings.axis_label_font_size if settings.axis_label_font_size is not None else settings.font_size_global
    tick_size = settings.tick_label_font_size if settings.tick_label_font_size is not None else settings.font_size_global
    legend_size = settings.legend_font_size if settings.legend_font_size is not None else settings.font_size_global
...
def apply_title(ax, base_title: str, settings: Optional[PlotCustomizationSettings]) -> None:
    ax.set_title(resolve_plot_title(base_title, settings))
...
def make_line_figure(
    ...
    resolved_width = resolve_series_line_width(series_name, line_width, plot_settings)
...
def render_plot_with_download(
    ...
    export_png: bytes
    if export_settings is not None and export_settings.use_custom_size:
        try:
            export_png = figure_to_png_bytes(
                fig,
                width_px=export_settings.width_px,
                height_px=export_settings.height_px,
                dpi=export_settings.dpi,
                tight_bbox=False,
            )
        except ValueError as exc:
            st.warning(f"Custom export size invalid ({exc}). Export used default image size.")
            export_png = figure_to_png_bytes(fig)
    else:
        export_png = figure_to_png_bytes(fig)
...
def plot_multi_series(
    ...
    for series in series_list:
        ...
        series_width = resolve_series_line_width(series.name, 1.2, plot_settings)
...
def run_tauc_fit_workflow(
    ...
    tauc_data_color = resolve_series_line_color(series_name, "#1d3557", plot_settings)
    tauc_data_width = resolve_series_line_width(series_name, 1.2, plot_settings)
    ax_tauc.plot(
        energy_ev,
        tauc_y,
    ...
    if mode in {"Excitation", "PL Emission"}:
        if peak_settings is None:
            ...
            if peaks is not None:
                ...
                signal_color = resolve_series_line_color(series.name, "#1d3557", plot_settings)
                signal_width = resolve_series_line_width(series.name, 1.2, plot_settings)
                ax_peak.plot(
                    series.x,
                    series.y_analysis,
                    color=signal_color,
...
def main() -> None:
    ...
    with st.expander("Plot Customization", expanded=False):
        ...
        font_family_input = st.text_input(
            "Font family",
            value=DEFAULT_REQUESTED_FONT_FAMILY,
        ...
        figure_heading = st.text_input(
            "Figure heading (optional)",
            value="",
            help="If provided, this heading is prepended to each plot title.",
        )
            ...
            with st.expander(f"Series: {series_name}", expanded=False):
                ...
                with color_col:
                    ...
                    selected_series_color = st.color_picker(
                        "Line color",
                        value=default_series_color_for_name(series_name),
        ...
        requested_preview_font = normalize_font_family(font_family_input)
    ...
    with st.expander("Export Image Settings", expanded=False):
        use_custom_export_size = st.checkbox(
            "Set custom export image size",
            value=False,
            help="When enabled, exported PNG size follows the selected pixel width/height.",
        )

        export_width_px = 2580
        export_height_px = 1440
        export_dpi = 300
        if use_custom_export_size:
            export_col1, export_col2, export_col3 = st.columns(3)
            with export_col1:
                export_width_px = int(
                    st.number_input("Export width (px)", value=2580, step=10, min_value=1)
                )
            with export_col2:
                export_height_px = int(
                    st.number_input("Export height (px)", value=1440, step=10, min_value=1)
                )
            with export_col3:
                export_dpi = int(st.number_input("Export DPI", value=300, step=10, min_value=1))
    ...
    requested_font_family = normalize_font_family(font_family_input)
    ...
    if use_custom_export_size and (export_width_px <= 0 or export_height_px <= 0 or export_dpi <= 0):
        plot_validation_messages.append(
            "Invalid export size settings: width, height, and DPI must be positive. Custom export size was not applied."
        )
        use_custom_export_size = False
            ...
            style_preview_rows.append(
                {
                    ...
                    "Line color": series_line_color_overrides.get(series_name, "[matplotlib default cycle]"),
                    "Line width": series_line_width_overrides.get(series_name, "[plot default]"),
                }
            )
        st.dataframe(pd.DataFrame(style_preview_rows), use_container_width=True, height=220)

    export_settings = ExportImageSettings(
        use_custom_size=use_custom_export_size,
        width_px=int(export_width_px) if use_custom_export_size else None,
        height_px=int(export_height_px) if use_custom_export_size else None,
        dpi=int(export_dpi),
    )
